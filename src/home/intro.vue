<template>
  <section v-if="setting('home.intro')" class="home-hero pt-12 lg:pt-0">
    <div
      v-if="section1Figure"
      class="home-hero-img hidden lg:block bg-blue-900"
      :style="`background-image: url(${section1Figure});`"
    />
    <div v-else class="home-hero-img hidden lg:block bg-blue-900" />
    <div class="mx-auto px-4 py-8 md:px-16 md:py-12">
      <h1
        v-if="section1Title"
        v-formatted-text="section1Title"
        class="font-normal tracking-tight leading-tight text-3xl mt-2 text-blue-900 lg:text-5xl"
      />
      <div
      ...
      <div v-if="section1Buttons" class="mt-4 md:mt-8">
        <template v-for="(button, index) in section1Buttons">
          <a :key="index" :href="button.link" :class="button.classes">
            {{ button.text }}

  ...
  >
    ...
    <div v-if="navConfig" class="relative z-50 flex flex-row md:hidden">
      <button
        ...
        @click="isOpen = !isOpen"
      >
        <svg class="h-6 w-6 fill-blue" viewBox="0 0 24 24">
          <path
            v-if="isOpen"
            fill-rule="evenodd"
        ...
        >
          ...
          >
            ...
            <factor-link
              ...
              :event="item.event"
              :target="item.target"
              class="mt-0 py-1 px-2 font-normal leading-loose text-xl transition-all text-blue-900 hover:bg-gray-100 hover:text-blue-900 md:hover:bg-transparent md:inline md:px-3 md:text-base md:hover:text-blue-500"
              @click="isOpen = !isOpen"
            >
